...
class Block2d(nn.Module):
  def __init__(self, in_channels, out_channels, time_emb_dim, up=False):
    ...
    self.time_conv = nn.Conv2d(in_channels, out_channels, kernel_size=1, padding="same") 
    self.bnorm_time = nn.BatchNorm2d(out_channels)
  ...
  def forward(self, x:torch.Tensor, t:Optional[torch.Tensor]=None):
    ...
    if not self.up:
        t = self.bnorm_time(F.silu(self.time_conv(t)))
        h = h + t
...
class RUNet(nn.Module):
  ...
  def __init__(self, in_channels=1, out_channels=1, time_emb_dim=64):
    ...
    # Initial projection
    self.conv0 = nn.Conv2d(in_channels, down_channels[0], 5, padding="same")
    ...
    # Final projection
    self.output = nn.Conv2d(up_channels[-1], out_channels, 7, padding="same")

  def build_rotational_embedding_mat(self, channels, theta): 
    mat = torch.zeros((channels, channels)).to(device)

    d_theta = theta / (channels//2)
    for i in range(channels//2): 
      idx = 2*i
      mat[idx+1, idx] = torch.sin(i * d_theta)
      mat[idx, idx+1] = -torch.sin(i * d_theta)
      mat[idx, idx] = torch.cos(i*d_theta)
      mat[idx+1, idx+1] = torch.cos(i*d_theta)

    return mat

  def compute_rotational_embeddings(self, x, theta): 
    n, c, h, w,  = x.shape
    t_emb = x.reshape(n, c, -1)# [N, C, Hxw]
    R = self.build_rotational_embedding_mat(c, theta)
    t_emb = R[None, ...] @ t_emb
    t_emb = t_emb.reshape(n, c, h, w) 
    return t_emb

  def forward(self, x, timestep):
    # Embedd time
    assert timestep.shape[0] == 2, f"timestep.shape[0] should be 2, got{timestep.shape[0]}"

    t1 = timestep[1]
    t0 = timestep[0] 

    theta = 2*np.pi *(t1-t0) - np.pi
    ...
    for down in self.downs:
        t_emb = self.compute_rotational_embeddings(x, theta)
        x = down(x, t_emb)
        residual_inputs.append(x)

    # rot_res_inputs = []
    # for embs in residual_inputs:
    #     n, c, h, w,  = embs.shape
    #     embs = embs.reshape(n, c, -1)# [N, C, Hxw]
    #     R = self.build_rotational_embedding_mat(c, theta)
    #     embs = R[None, ...] @ embs
    #     embs = embs.reshape(n, c, h, w) 
    #     rot_res_inputs.append(embs)
...
class RPhiNet(nn.Module):
  def __init__(self, input_channels=2):
    super(RPhiNet, self).__init__()
    self.net = RUNet(input_channels,2).to(device)
